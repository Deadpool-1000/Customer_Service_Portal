...
class HelpDeskTicketSection:
    ...
    def unresolved_tickets(self):
        with DatabaseConnection() as conn:
            ...
            all_tickets = t_dao.get_all_raised_tickets(self.helpdesk.dept_id)
        ...
        unresolved_ticket_functionalities = {
            'r': lambda: self.resolve_ticket_handler(all_tickets),
    ...
    def resolved_tickets(self):
        with DatabaseConnection() as conn:
            t_dao = TicketDAO(conn)
            all_tickets = t_dao.get_all_in_progress_tickets(self.helpdesk.dept_id)
    ...
    def closed_tickets(self):
        with DatabaseConnection() as conn:
            t_dao = TicketDAO(conn)
            all_tickets = t_dao.get_all_closed_tickets(self.helpdesk.dept_id)
    ...
    def resolve_ticket_handler(self, all_tickets):
    ...
    def view_ticket_detail_handler(self, all_tickets):
        ...
        if found_ticket[0].status == HelpdeskTicketConfig.IN_PROGRESS:
            ...
            if user_choice == 'r':
                self.resolve_ticket_handler(all_tickets)
            else:
                return
